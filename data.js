function getRemoteJSON(url, throwIfNotFound) {
  return fetch(url).then(function (response) {
    ...
    throw new Error(
      "Impossible de récupérer les données demandées : " + response.status
    );
  });
}
...
function getCommunes(codeDepartement) {
  return getRemoteJSON(
    `https://geo.api.gouv.fr/departements/${codeDepartement}/communes?geometry=contour&format=geojson&type=commune-actuelle`
  ).then(function (communes) {
    // Pour Paris, Lyon, Marseille, il faut compléter avec les arrondissements
    if (["75", "69", "13"].includes(codeDepartement)) {
      return getRemoteJSON(
        "/donneesgeo/arrondissements_municipaux-20180711.json"
    ...
    return {
      ...
      features: sortByNom(communes.features),
    };
  });
}
...
function getMutations(codeCommune, idSection, startDate, endDate) {
  return getRemoteJSON(
    `/api/mutations3/${codeCommune}/${idSectionToCode(idSection)}`
  ).then(function (data) {
    return data.mutations.filter(function (m) {
      return (
        m.date_mutation >= startDate &&
        m.date_mutation <= endDate &&
        m.id_parcelle.startsWith(idSection)
      );
    });
  });
}

var communesMappingPromise = getRemoteJSON(
  "/donneesgeo/communes-mapping.json",
  true
);
...
function getCadastreLayer(layerName, codeCommune) {
  return communesMappingPromise.then(function (communesMapping) {
    var communesToGet =
      codeCommune in communesMapping
        ? communesMapping[codeCommune]
        : [codeCommune];
    return Promise.all(
      communesToGet.map(function (communeToGet) {
        return getRemoteJSON(
...
function getParcelles(codeCommune, idSection) {
  return getCadastreLayer("parcelles", codeCommune).then(function (
    featureCollection
  ) {
    return {
      type: "FeatureCollection",
...
function getSections(codeCommune) {
  return getCadastreLayer("sections", codeCommune).then(function (
    featureCollection
  ) {
    var features = featureCollection.features;
    var hasMultiplePrefixes = features.some(function (f) {
      return (
        f.properties.commune !== codeCommune || f.properties.prefixe !== "000"
      );
    });
    features.forEach(function (f) {
      ...
      var labelPrefix =
        f.properties.commune === codeCommune
          ? f.properties.prefixe
          : f.properties.commune.substr(2);
